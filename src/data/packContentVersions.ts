import { useCustomPicbookStore } from '../state/customPicbookStore'

/**
 * 팩 본문·연출·이미지를 수정할 때마다 해당 bookId의 버전을 올리면,
 * 이미 구매(잠금 해제)한 사용자에게도 재생 시 최신 팩이 자동 적용된다.
 */
export const PACK_CONTENT_VERSIONS: Record<string, string> = {
  'demo-separation-three-powers': '15',
  'elementary-proverbs': '32',
  'tortoise-and-hare': '4',
}

export function getPackContentVersion(bookId: string): string {
  const custom = useCustomPicbookStore.getState().books.find((b) => b.id === bookId)
  if (custom) return custom.contentVersion
  return PACK_CONTENT_VERSIONS[bookId] ?? '1'
}
